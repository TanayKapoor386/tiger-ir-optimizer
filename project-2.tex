...
\title{CS 4240: Compilers and Interpreters, Project 2, Spring 2026\\
\Large
Assigned: February 16, 2026, 10\% of course grade\\
(100 points total)\\
Due by March 16, 2026 11:59pm on Gradescope
}
\preauthor{}
\postauthor{}
\author{}
...
\begin{document}
\lhead{CS 4240, Spring 2026}
\rhead{Project 2}
\maketitle
\thispagestyle{fancy}

\section{Project Description}
In this project, you will build a simple optimizer (middle end) for Tiger-IR. 
The reference manual for Tiger-IR can be found on the course Canvas Files section.
...
obtain a reduced dynamic instruction count. More specifically:
\begin{itemize}
    \item \textit{Static Instruction Count:} It is the total number of instructions present in the compiled IR. It is a spatial measurement of the program's \textit{size}.
    \item \textit{Dynamic Instruction Count:} It is the total number of instructions actually executed by the IR interpreter/processor during a specific run of the program. It is a measurement of the program's \textit{execution time}.
\end{itemize}
Both of these metrics could be understood using a simple example given below:
...
\begin{minipage}{0.45\textwidth}
    ...
    \end{center}
\end{minipage}%
\hfill% % Fills space between minipages
\begin{minipage}{0.45\textwidth}
\begin{itemize}
    \item Static Count: 8 instructions.
    \item Dynamic Count: Approximately 500+ instructions (the loop body executes 100 times, plus the initial setup and exit).
\end{itemize}
\end{minipage}
...
performance goals outlined below in Section~\ref{sec:test}. For correctness, any program $P$ that is subject to an optimization $O$, the output program $O(P)$ must be I/O equivalent to $P$. That is:
\[ \forall x, P(x) \equiv (O(P))(x) \]
...
\begin{itemize}
\item Based on the algorithm in Lecture slides (without reaching definitions)
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Based on the algorithm in  Lecture slides (with reaching definitions)
\end{itemize}
\end{itemize}
...
\url{https://github.gatech.edu/CS4240-Spring26/Project-2}
...
If you can't access the repository, please make a private post on Ed to let the TAs know.

\subsection{Helper Code}

The Java helper code is located under \texttt{/Project-2/materials/src/ir}.
...
which is demonstrated in \texttt{/Project-2/materials/src/Demo.java}, but you are
...
The source code of the IR interpreter is in \texttt{/Project-2/materials/src/IRInterpreter.java}.
It also uses the helper code.
Please refer to \texttt{/Project-2/materials/README.md} for more information about how to build and use it.
...
The collaboration policy for Project-2 is same as collaboration policy of Project-1.
